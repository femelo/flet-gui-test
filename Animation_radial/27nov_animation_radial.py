...
from radial_animation import get_radial_animation  # Importeer de animatie
...
def process_gui_message(data, page, message_store):
    try:
        ...
        if msg_type == "mycroft.events.triggered" and event_name == "recognizer_loop:wakeword":
            log_message("Wakeword gedetecteerd! Toon animatie.")
        
            # Maak de animatie zichtbaar
            page.animatie_container.visible = True
            page.update()
        
            # Verberg de animatie na enkele seconden (indien gewenst)
            def hide_animation():
                time.sleep(5)  # Animatie wordt 5 seconden getoond
                page.animatie_container.visible = False
                page.update()
...
def update_home_page(page):
    ...
    # Maak de animatiecontainer standaard verborgen
    animatie_container = ft.Container(
        content=get_radial_animation(page),
        width=150,
        height=150,
        padding=10,
        alignment=ft.alignment.top_left,
        visible=False  # Begin als verborgen
    )
    ...
    home_view = ft.View(
        "/",
        controls=[
            ft.Stack([achtergrond_container, animatie_container, overlay], expand=True),
        ],
    )
    ...
    # Sla de animatiecontainer op in de pagina om er later naar te kunnen verwijzen
    page.animatie_container = animatie_container
